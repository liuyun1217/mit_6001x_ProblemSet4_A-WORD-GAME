...
def getWordScore(word, n):
    ...
    num = 0
    for i in word:
        for values in SCRABBLE_LETTER_VALUES:
            if i == values:
                result = result + SCRABBLE_LETTER_VALUES[values]
                num += 1
    if num==n:
        result = result*num+50 #all words were used
    elif num<n:
        result = result*num
    return result
...
def updateHand(hand, word):
    ...
    for k in hand:
        wordDic[k] = hand[k]-wordDic.get(k,0) #if no k ,retuen a zero
